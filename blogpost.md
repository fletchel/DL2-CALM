...
Moreover, the maximal sequence length of the input to the **attention-based classifier** is limited by the depth of the original model, this method has a time complexity of $\mathcal{O}(D N^2)$, where $N$ denotes the total number of layers in the full model. 
